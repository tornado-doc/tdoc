...
OLD_TEST = """  // Visible PLACEHOLDER copy is allowed while drafting, but it must sit inside
...
NEW_TEST = """  // v37 filled the six quote slots. Visible PLACEHOLDER copy is no longer
...
def update_test() -> None:
    ...
    if OLD_TEST not in src:
        raise SystemExit("landing test placeholder block missing")
    path.write_text(src.replace(OLD_TEST, NEW_TEST))
