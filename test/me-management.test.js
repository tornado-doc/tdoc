...
t('Create a doc is a two-card fork rendered by the shared component', () => {
  ...
  assert(/copyText\(FIRST_DOC_RECIPE\)/.test(choice), 'the recipe is not copyable');
  assert(/className="tdoc-recipe-wrap"/.test(choice), 'shared recipe treatment missing');
  assert(/copied === true \? 'Copied' : copied === false \? 'Select & copy' : 'Copy'/.test(choice), 'Copy feedback state missing (three states: idle, copied, clipboard refused)');
