...
async function putSession(env, login, sid) {
  // parseCookie only accepts hex tdoc_sid values.
  const id = sid || [...login].map((c) => c.charCodeAt(0).toString(16).padStart(2, '0')).join('').slice(0, 24).padEnd(8, '0');
  await env.META.put(`session:${id}`, JSON.stringify({
    login, avatar_url: '', name: login, created: new Date().toISOString(),
  }));
  return `tdoc_sid=${id}`;
}
